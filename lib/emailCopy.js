...
// Pick a natural greeting name: "Dr. Jane Smith" -> "Dr. Smith", "Jane Smith" -> "Jane".
function greetingName(owner) {
  const parts = String(owner || '').trim().split(/\s+/).filter(Boolean);
  if (!parts.length) return '';
  const TITLE = /^(dr|mr|mrs|ms|miss|prof|mx)\.?$/i;
  if (TITLE.test(parts[0])) return parts[0].replace(/\.?$/, '.') + ' ' + parts[parts.length - 1];
  return parts[0];
}
function buildEmailPrompt(lead, stage) {
  const guide = STAGE_GUIDE[stage] || STAGE_GUIDE.initial;
  const firstName = greetingName(lead.owner_name);
